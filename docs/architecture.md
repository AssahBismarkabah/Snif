...
filesystem provides conventions, file contents, and related code. The configured
LLM provider handles model execution through an OpenAI-compatible API. CI
pipelines or platform webhooks invoke `snif review`, `snif index`, or
`snif eval`.
...
`execution` calls the configured LLM provider through a provider-neutral HTTP
interface. The provider is configured in `.snif.json` and can be any
OpenAI-compatible endpoint — AWS Bedrock, OpenAI, Azure, or a self-hosted
model. The execution layer contains no product policy — its job is to call the
...
execution adapter calls the configured LLM provider and returns the raw model
response. The parser
...
summary is generated by the LLM through the configured provider. This is the expensive part of
